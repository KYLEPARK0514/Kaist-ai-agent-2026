    ...
    <div className="flex h-screen flex-col bg-gray-50 font-sans text-gray-900">
      ...
      <main className="flex flex-1 overflow-hidden">
        ...
        <div className="hidden w-full md:flex">
          ...
          {/* Right panel — Chat Layout (conversation list + chat window) */}
        ...
        <div className="flex w-full flex-col md:hidden">
          <div className="flex-1 overflow-hidden bg-white">
            {activeTab === 'documents' ? <DocumentManager /> : <ChatLayout />}
          </div>

          <nav className="flex border-t border-gray-200 bg-white" aria-label="Main navigation">
            <button
              onClick={() => setActiveTab('documents')}
              className={`flex flex-1 flex-col items-center gap-0.5 py-2 text-xs font-medium transition-colors ${activeTab === 'documents' ? 'text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
              aria-current={activeTab === 'documents' ? 'page' : undefined}
            >
              <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={activeTab === 'documents' ? 2 : 1.5} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
              Documents
            </button>
            ...
            <button
              ...
              className={`flex flex-1 flex-col items-center gap-0.5 py-2 text-xs font-medium transition-colors ${activeTab === 'chat' ? 'text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
